...
///
/// `ShaderAttribute` specifies per-node data that can be passed to shaders.
...
/// let colorAttribute = ShaderAttribute(name: "a_color", type: .float4)
/// let intensityAttribute = ShaderAttribute(name: "a_intensity", type: .float)
///
/// // Create a shader with these attributes
/// let shader = Shader(source: shaderCode, attributes: [colorAttribute, intensityAttribute])
/// ```
public final class ShaderAttribute: Sendable {
